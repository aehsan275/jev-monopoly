...
import { ArrowLeft, ChevronLeft, ChevronRight, Clock3, LoaderCircle, Play, RotateCcw, ShieldAlert, Trophy } from "lucide-react";
import { GameBoard } from "@/app/play-game";
...
export default function ReplayView({ id }: { id: string }) {
  ...
  return <main className="replay-shell"><header className="game-topbar"><Link className="brand-lockup" href="/"><span className="brand-mark">J</span><span><strong>JEV / MONOPOLY</strong><small>SHARED REPLAY</small></span></Link><div className="table-status"><Clock3 size={15} />View only<small>Expires {new Date(expiresAt).toLocaleDateString()}</small></div><nav><Link href="/"><ArrowLeft size={14} /> Play</Link><a href="/benchmarks">Benchmarks</a></nav></header>
